function E_1_by_F_1_ = GetE_1_by_F_1( shape , SpatialPointsPerDimension,  GN_ , nu, gamma, Gamma, Omega, lambda, phi_, deltaJ, GJTrend_, thetaC, thetaF, thetaH, thetaL, thetaN, kappa, alpha, GYTrend_, GSRKTrend_, Xi_LEAD_, deltaK, Phi2, GSPTrend_ , GmuNTrend_, GUTrend_, psi1, psi2, psi3, tau_, dBar, beta_ , varsigma)
        ...
        while crit<1
            ...
            while count<=error_count   
                try  
                    ...
                    E_x_by_F_x_N_x_F_x_K_x_H_x_Q_x_SN_xx_ = exp( lsqnonlin(steadystate_lsqnonlin) );
                    crit = homotopy;
                    figure;subplot(3,2,1);surf(reshape(E_x_by_F_x_N_x_F_x_K_x_H_x_Q_x_SN_xx_( 1:SpatialPoints ),[SpatialPointsPerDimension,SpatialPointsPerDimension])); title('E_by_F');
...
function Residual = GetResidual( E_x_by_F_x_N_x_F_x_K_x_H_x_Q_x_SN_xx_, shape, SpatialPointsPerDimension, d, GN_, nu, gamma, Gamma, Omega, lambda, phi_, deltaJ, GJTrend_, thetaC, thetaF, thetaH, thetaL, thetaN, kappa, alpha, GYTrend_, GSRKTrend_, Xi_LEAD_, deltaK, Phi2, GSPTrend_, GmuNTrend_, GUTrend_, psi1, psi2, psi3, tau_, dBar, beta_, varsigma, homotopy )
    ...
    for x=1:SpatialPoints
        Utilde_x_( x ) = Get_Utilde(  SpatialPointsPerDimension, x ); 
    end
    Utilde_x_ = 1 + homotopy*(Utilde_x_-1);
