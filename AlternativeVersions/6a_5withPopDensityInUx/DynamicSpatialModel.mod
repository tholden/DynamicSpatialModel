...
model;
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        * ( 1 / 2 * Omega ^ 2 - 1 / 2 * log( N@{Index1}_LAG * ( 1 - L_LAG ) / ( N_LAG * ( 1 - L@{Index1}_LAG ) ) ) ^ 2 ) ^ thetaN
...
%        ) );
        muN@{Index1} = beta * ( muN@{Index1}_LEAD * GN_LEAD + U@{Index1}_LEAD ^ ( 1 - varsigma ) + ( 1 - varsigma ) * U@{Index1}_LEAD ^ ( 1 - varsigma ) * (
            thetaH * ( H@{Index1}_LEAD / N@{Index1} ) ^ ( 1 + nu ) / ( 1 / ( 1 + nu ) * Gamma ^ ( 1 + nu ) - 1 / ( 1 + nu ) * ( H@{Index1}_LEAD / N@{Index1} ) ^ ( 1 + nu ) )
            - thetaN * log( N@{Index1} * (1-L) / (N * (1-L@{Index1})) ) / ( 1 / 2 * Omega ^ 2 - 1 / 2 * log( N@{Index1} * (1-L) / (N * (1-L@{Index1})) ) ^ 2 )
            + psi1 * SN@{Index1}_LEAD / ( N@{Index1} - SN@{Index1}_LEAD )
            - ( thetaC + thetaF + thetaL + psi3 )
        ) );
    @#endfor
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        Xi_LEAD * SRD@{Index1}_LEAD * ( 1 + ( thetaN / thetaL) * ( D@{Index1}_LEAD / (1-L@{Index1}) ) * log( N@{Index1} * (1-L) / (N * (1-L@{Index1})) ) / ( 1 / 2 * Omega ^ 2 - 1 / 2 * log( N@{Index1} * (1-L) / (N * (1-L@{Index1})) ) ^ 2 ) ) = Xi_LEAD * SRL@{Index1}_LEAD - PhiL * ( L@{Index1} / L@{Index1}_LAG - 1 ) / L@{Index1}_LAG + Xi_LEAD * PhiL * ( L@{Index1}_LEAD / L@{Index1} - 1 ) * ( L@{Index1}_LEAD / ( L@{Index1}^2 ) );
