...
parameters UtilityParamSum theta beta_r_by_beta;
...
// GSRKTrend  = GYTrend ^ ( - ( gamma + lambda ) / ( 1 + lambda ) );
   ...
   @#endfor
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        phi * SP@{Index1} = (lambda / ( 1 + lambda ) * ( 1 + lambda ) ^ ( - 1 / lambda ) * SP@{Index1} ^ ( - 1 / lambda ) * YBar@{Index1}) + ( 1 - deltaJ ) * Xi_LEAD * phi_LEAD * SP@{Index1}_LEAD;

        [name = 'Z demand'] Z@{Index1} = ZF@{Index1} + phi * ( J@{Index1} - ( 1 - deltaJ ) * J@{Index1}_LAG ) + J@{Index1} * ( ( 1 + lambda ) * SP@{Index1} ) ^ ( - ( 1 + lambda ) / lambda ) *  YBar@{Index1};
    @#endfor

    [name = 'Euler equation'] 1 = R * Xi_LEAD;
...
@#else
    ...
    steady_state_model;
        ...
        F_1_ = GetF_1_( A_1_, Nr_1_, Nr_1_LAG_, Ns_1_, Ns_1_LAG_, nu, gamma, Gamma, L_1_, lambda, phi_, deltaJ, GJTrend_, AverageTransportCost_, thetaL, thetaC, thetaF, thetaH, kappa, alpha, GYTrend_, GSRKTrend_, P_1_Over_Q_1_, Xi_LEAD_, deltaK, Phi2, GSPTrend_, GZTrend_, GPTrend_, GYBarTrend_, GOmega1Trend_);
        K_1_ = GetK_1_( 1 );
        H_1_ = GetH_1_( 1 );
        Q_1_ = GetQ_1_( 1 );
...
@#else
    stoch_simul( order = 1, pruning, irf = 400, periods = 0, nocorr, nofunctions, nodisplay, nograph, irf_shocks = ( epsilon_AT_2_2, epsilon_GA, epsilon_GN, epsilon_tau, epsilon_phi, epsilon_beta ) ); // k_order_solver
@#endif
