...
@#for Point in 1 : SpatialNumPoints
    ...
    @#define EndoVariables = EndoVariables + [ "N_LAG" + Index1, "0", "Inf", "GN" ]
    ...
    @#define EndoVariables = EndoVariables + [ "U_LEAD" + Index1, "0", "Inf", "GUTrend" ]
...
@#define AggregatedVariables = [ "Y", "C", "K", "I", "F", "J", "L", "H", "SN", "SD", "U" ]
...
parameters UtilityParamSum Pi_star xi rho_R theta_y theta_pi;
...
model;
    ...
    @#for Point1 in 1 : SpatialNumPoints
        @#define Index1 = IndicesStringArray[Point1]
        + Weight@{Index1} * N_LAG@{Index1}_LAG
    @#endfor
    ;
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        N_LAG@{Index1} = N@{Index1}_LAG; 
        U_LEAD@{Index1} = U@{Index1}_LEAD;

        U@{Index1} = ( C@{Index1} / N@{Index1}_LAG ) ^ thetaC
        * ( E@{Index1} / N@{Index1}_LAG ) ^ thetaF
        * ( ( D@{Index1} ) / N@{Index1}_LAG ) ^ thetaL
        * ( 1 / ( 1 + nu ) * Gamma ^ ( 1 + nu ) - 1 / ( 1 + nu ) * ( H@{Index1} / N@{Index1}_LAG ) ^ ( 1 + nu ) ) ^ thetaH
        * ( 1 / 2 * Omega ^ 2 - 1 / 2 * log( N@{Index1}_LAG * N_LAG_LAG / ( N_LAG * N_LAG@{Index1}_LAG ) ) ^ 2 ) ^ thetaN
        ...
        * exp( psi3 * ( 0
        ...
        muN@{Index1} = beta * ( muN@{Index1}_LEAD * GN_LEAD + U@{Index1}_LEAD ^ ( 1 - varsigma ) + ( 1 - varsigma ) * U@{Index1}_LEAD ^ ( 1 - varsigma ) * (
            ...
            - thetaN * log( N@{Index1} * N_LAG / ( N * N_LAG@{Index1} ) ) / ( 1 / 2 * Omega ^ 2 - 1 / 2 * log( N@{Index1} * N_LAG / ( N * N_LAG@{Index1} ) ) ^ 2 )
            + psi1 * SN@{Index1}_LEAD / ( N@{Index1} - SN@{Index1}_LEAD )
            - ( thetaC + thetaF + thetaL + psi3 ) 
        ) 
            + beta_LEAD * ( 1 - varsigma ) * U_LEAD@{Index1}_LEAD ^ ( 1 - varsigma ) * ( N@{Index1}_LEAD / N@{Index1} ) * thetaN * log( N@{Index1}_LEAD * N / ( N_LEAD * N@{Index1} ) ) / ( 1 / 2 * Omega ^ 2 - 1 / 2 * log( N@{Index1}_LEAD * N / ( N_LEAD * N@{Index1} ) ) ^ 2 )
        );
    @#endfor
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        SPJ1@{Index1} = ( xi * (1-deltaJ) * ( J@{Index1}_LAG / J@{Index1} ) * ( SPJ1@{Index1}_LAG / Pi )^( -1 / lambda ) + ( 1 - xi * (1-deltaJ) * J@{Index1}_LAG / J@{Index1} ) * P_star@{Index1}^( -1 / lambda ) )^( -lambda );
        SPJ2@{Index1} = ( xi * (1-deltaJ) * ( J@{Index1}_LAG / J@{Index1} ) * ( SPJ2@{Index1}_LAG / Pi )^( -(1+lambda) / lambda ) + ( 1 - xi * (1-deltaJ) * J@{Index1}_LAG / J@{Index1} ) * P_star@{Index1}^( -(1+lambda) / lambda ) )^( -lambda/(1+lambda) );
        ...
        #SRD@{Index1}_LEAD = thetaL * E@{Index1}_LEAD / ( thetaF *  D@{Index1}_LEAD ); 
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        Omega1@{Index1} = YBar@{Index1} * SP@{Index1} + xi * Xi_LEAD * ( Pi_LEAD^((1+lambda)/lambda) ) * Omega1@{Index1}_LEAD; 
        Omega2@{Index1} = YBar@{Index1} + xi * Xi_LEAD * ( Pi_LEAD^(1/lambda) ) * Omega2@{Index1}_LEAD; 
        ...
        varphi1@{Index1} = (1-deltaJ) * xi * Xi_LEAD * ( Pi_LEAD^(1/lambda) ) * ( YBar@{Index1}_LEAD +  varphi1@{Index1}_LEAD );
        varphi2@{Index1} = (1-deltaJ) * xi * Xi_LEAD * ( Pi_LEAD^((1+lambda)/lambda) ) * ( YBar@{Index1}_LEAD * SP@{Index1}_LEAD +  varphi2@{Index1}_LEAD );
        varphi3@{Index1} = (1-deltaJ) * xi * Xi_LEAD * ( pis@{Index1}_LEAD +  varphi3@{Index1}_LEAD );
        phi * SP@{Index1} = pis@{Index1};
        
        Z@{Index1} = ZF@{Index1} + phi * ( J@{Index1} - ( 1 - deltaJ ) * J@{Index1}_LAG ) + J@{Index1} * SPJ2@{Index1}^(-(1+lambda)/lambda) *  YBar@{Index1};
    @#endfor
...
%    );
    ...
    R = R_LAG^rho_R * ( STEADY_STATE(R) * ( Y / STEADY_STATE(Y) )^theta_y * ( PiC / Pi_star )^theta_pi  )^( 1 - rho_R );
...
%        Pi_ = PiC_ / GPTrend_;
        ...
        F_1_ = GetF_1_( A_1_, N_1_, N_1_LAG_, nu, gamma, Gamma, L_1_, lambda, phi_, deltaJ, GJTrend_, AverageTransportCost_, thetaC, thetaF, thetaH, kappa, alpha, GYTrend_, GSRKTrend_, P_1_Over_Q_1_, Xi_LEAD_, deltaK, Phi2, GSPTrend_, GZTrend_, xi, GPTrend_, GYBarTrend_, GOmega1Trend_, Pi_);
        K_1_ = GetK_1_( 1 );
        H_1_ = GetH_1_( 1 );
        Q_1_ = GetQ_1_( 1 );
    ...
    J_1_ = ( SPJ1_1_ * AverageTransportCost_ / P_1_ ) ^ ( 1 / lambda );
        ...
        * ( 1 / ( 1 + nu ) * Gamma ^ ( 1 + nu ) - 1 / ( 1 + nu ) * ( H_1_ / N_1_LAG_ ) ^ ( 1 + nu ) ) ^ thetaH
        ...
        @#for Point1 in 1 : SpatialNumPoints
            ...
            H@{Index1}_ = H_1_;
            N@{Index1}_ = N_1_;
            N_LAG@{Index1}_ = N_1_LAG_;
            SN@{Index1}_ = SN_1_;
            SD@{Index1}_ = SD_1_;
            muN@{Index1}_ = muN_1_;
            U@{Index1}_ = U_1_;
            U_LEAD@{Index1}_ = U_1_LEAD_;
