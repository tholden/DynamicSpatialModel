...
model;
    ...
    @#for Point1 in 1 : SpatialNumPoints
        ...
        * ( Omega * N_LAG / N@{Index1}_LAG - 1 ) ^ thetaN
        ...
        * exp( psi3 * ( 0
        ...
        muN@{Index1} = beta * ( muN@{Index1}_LEAD * GN_LEAD + U@{Index1}_LEAD ^ ( 1 - varsigma ) + ( 1 - varsigma ) * U@{Index1}_LEAD ^ ( 1 - varsigma ) * (
            ...
            - thetaN * Omega * N / ( Omega * N - N@{Index1} )
...
@#else
    ...
    steady_state_model;
        ...
        * ( 1 / ( 1 + nu ) * Gamma ^ ( 1 + nu ) - 1 / ( 1 + nu ) * ( H_1_ / N_1_LAG_ ) ^ ( 1 + nu ) ) ^ thetaH
         * ( Omega * N_LAG_ / N_1_LAG_ - 1 ) ^ thetaN
        ...
        H_1_LEAD_ = H_1_ * GN_;
        muN_1_ = beta_ * ( U_1_LEAD_ ^ ( 1 - varsigma ) + ( 1 - varsigma ) * U_1_LEAD_ ^ ( 1 - varsigma ) * ( thetaH * ( H_1_LEAD_ / N_1_ ) ^ ( 1 + nu ) / ( 1 / ( 1 + nu ) * Gamma ^ ( 1 + nu ) - 1 / ( 1 + nu ) * ( H_1_LEAD_ / N_1_ ) ^ ( 1 + nu ) ) + psi1 * SN_1_ * GN_ / ( N_1_ - SN_1_ * GN_ ) - thetaN * Omega * N_ / ( Omega * N_ - N_1_ ) - ( thetaC + thetaF + thetaL + psi3 ) ) ) / ( 1 - beta_ * GmuNTrend_ * GN_ );
